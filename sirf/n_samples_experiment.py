...
def experiment(workers = 1, n_runs = 10, k = 16, env_size = 15, gam = 0.995, lam = 0., eps = 1e-5,  
    ...
    d_loss_data = {}
    for k in d_loss_funcs.iterkeys():
        d_loss_data[k] = numpy.zeros((len(n_samples), n_runs, len(training_methods)))
        ...
        for i,n in enumerate(n_samples):
            ...
            for r in xrange(n_runs):
                ...
                for j,tm in enumerate(training_methods):
                    ...
                    condor_job((i,r,j), d_loss_funcs, theta_init, dim, k, beta_ratio, partition, S, R, S_test, R_test, Mphi, Mrew,
    ...
    plot_styles = ['r-', 'b-', 'g-']
    for i,(k,mat) in enumerate(d_loss_data.items()):
...
def condor_job(ind_tuple, d_loss_funcs, method, theta_init, dim, k, beta_ratio,
    ...
    for key,fun in d_loss_funcs.items():
        if 'test' in k:
            d_batch_loss[key] = fun(bb.theta, S_test, R_test, Mphi, Mrew) / float(n) 
        elif 'true' in k:
