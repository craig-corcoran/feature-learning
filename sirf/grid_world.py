...
class SIRFObjectiveFn:
    ...
    def __init__(self, W, rewards, states, env, mod_wt):
        ...
        X_all = scipy.sparse.lil_matrix((self.n_samples+1, W.shape[0]), dtype = numpy.int)
    ...
    def _precompute():
        ...
        STD = numpy.sqrt(STD)
        D = scipy.sparse.dia_matrix((1./STD, 0), (n_features, n_features))
        
        self.PHI_p_norm = self.PHI_p * D

    def _reward_gradient():
    ...
    def get_gradient(W):
        ...
        return dW_rew + self.mod_wt * dW_mod 
